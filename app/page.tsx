...
export default function Page() {
  ...
  const [filters, setFilters] = useState<FiltersState>({ company: '', types: [], salaryMin: null, salaryMax: null, maxDistanceKm: 50 })
  ...
  const jobs: Job[] = useMemo(() => {
    return (jobsData as Job[]).map(j => ({ ...j, distanceKm: haversineDistanceKm(JAIPUR.lat, JAIPUR.lon, j.location.lat, j.location.lon) } as any))
  }, [])
  ...
  function resetFilters() {
    setFilters({ company: '', types: [], salaryMin: null, salaryMax: null, maxDistanceKm: 50 })
  }

  // dashboard jobs are those not applied and matching filters
  const dashboardJobs = useMemo(() => {
    return jobs.filter(j => !applied.has(j.id)).filter(j => {
      ...
      if (filters.salaryMin != null && j.salary < filters.salaryMin) return false
      if (filters.salaryMax != null && j.salary > filters.salaryMax) return false
      if ((j as any).distanceKm > filters.maxDistanceKm) return false
      return true
    })
  }, [jobs, applied, filters])

  const appliedJobs = useMemo(() => jobs.filter(j => applied.has(j.id)), [jobs, applied])
    ...
    <div>
      <div className="header">
        ...
        <div className="tabs">
          ...
          <button className={"tab " + (tab==='applied' ? 'active':'')} onClick={()=>setTab('applied')}>
            <span>Applied</span>
      ...
      {tab === 'dashboard' ? (
        <div className="panel">
          ...
          <div className="results">
            <div style={{marginBottom:8}} className="small">Showing {mounted ? dashboardJobs.length : ''} jobs</div>
            {dashboardJobs.length === 0 ? (
              <div className="empty">No jobs match your filters</div>
            ) : (
              <JobList jobs={dashboardJobs as any} onApply={apply} appliedIds={applied} />
            )}
          </div>
        </div>
      ) : (
